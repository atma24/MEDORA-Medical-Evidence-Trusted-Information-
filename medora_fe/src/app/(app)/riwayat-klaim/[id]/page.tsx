...
export default function DetailKlaimPage() {
  ...
  const handleDownloadPdf = async () => {
    if (!claim || isDownloading) return;
    setIsDownloading(true);
    try {
      const res = await api.get(`/claims/${id}/pdf`, { responseType: 'blob' });
      const blob = new Blob([res.data], { type: 'application/pdf' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      // coba ambil filename dari header, fallback ke id
      const disposition = (res.headers as any)?.['content-disposition'] || '';
      const match = disposition.match(/filename="?([^"]+)"?/);
      a.download = match ? match[1] : `MEDORA-CLM-${claim.id}.pdf`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      window.URL.revokeObjectURL(url);
    } catch (error: any) {
      const msg = error?.response?.data?.message || 'Gagal mengunduh PDF. Silakan coba lagi.';
      // jika blob error, coba parse json
  ...
  const handleShare = async () => {
    ...
    const shareTitle = `MEDORA - ${claim?.text ?? 'Klaim'}`;
    try {
      if (navigator.share) {
        await navigator.share({ title: shareTitle, text: claim?.text ?? '', url: shareUrl });
      } else if (navigator.clipboard) {
        await navigator.clipboard.writeText(shareUrl);
        alert('Tautan disalin ke clipboard!');
      } else {
        prompt('Salin tautan:', shareUrl);
      }
    } catch { /* user cancel share */ }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="px-3.5 py-2 bg-white border border-gray-200 text-red-600 rounded-xl text-xs font-bold hover:bg-red-50 transition flex items-center gap-2 shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
          >
          ...
          <button onClick={handleShare} className="px-4 py-2 bg-[#0F172A] text-white rounded-xl text-xs font-bold hover:bg-slate-800 transition flex items-center gap-2 shadow-sm">
