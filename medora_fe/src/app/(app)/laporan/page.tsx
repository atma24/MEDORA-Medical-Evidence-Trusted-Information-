...
export default function LaporanAnalitikPage() {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      const period = PERIOD_MAP[filterOption] ?? 'month';
      const res = await api.get('/review/report/pdf', { params: { period }, responseType: 'blob' });
      ...
      a.download = m ? m[1] : `MEDORA-Laporan-${filterOption}.pdf`;
      ...
      if (e?.response?.data instanceof Blob) {
        try { const t = await (e.response.data as Blob).text(); const j = JSON.parse(t); alert(j.message || 'Gagal export PDF'); } catch { alert('Gagal export PDF'); }
      } else {
        alert(e?.response?.data?.message || 'Gagal export PDF. Silakan coba lagi.');
      }
    } finally {
