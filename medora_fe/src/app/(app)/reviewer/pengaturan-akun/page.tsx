...
export default function ReviewerPengaturanAkunPage() {
  ...
  // States Toggle Notifikasi (agar Switch-nya bisa di-klik)
  ...
  // State Form Pribadi (Dr. Rauf)
  const [formData, setFormData] = useState({
    nama: 'Dr. Rauf, Sp.PD',
    email: 'Snowfreze@gmail.com',
    telepon: '+62 812-3456-7890',
    bio: 'Dokter Spesialis Penyakit Dalam',
    sip: 'SIP-442/109/DU/2024',
    spesialisasi: 'Penyakit Dalam',
    instansi: 'RS Pusat Pertamina Jakarta'
  });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-200 flex flex-col items-center text-center sticky top-6">
            ...
            <h3 className="text-[20px] font-extrabold text-slate-800 mb-1">{formData.nama}</h3>
            ...
            <p className="text-[13px] text-gray-400 font-medium pt-4 border-t border-gray-100 w-full">Bergabung: 2026</p>
          </div>
        </div>
              ...
              <form onSubmit={handleSave}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  <div>
                    ...
                    <input type="text" value={formData.nama} onChange={(e) => setFormData({...formData, nama: e.target.value})} className="w-full bg-[#F8FAFC] border border-gray-200 rounded-lg px-4 py-3 text-[14px] text-slate-800 focus:bg-white focus:border-[#253E6B] outline-none" />
                  </div>
                  <div>
                    <label className="block text-[13px] font-bold text-slate-700 mb-2">Alamat Email</label>
                    <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full bg-[#F8FAFC] border border-gray-200 rounded-lg px-4 py-3 text-[14px] text-slate-800 focus:bg-white focus:border-[#253E6B] outline-none" />
                  </div>
                </div>
                ...
                <div className="border-t border-gray-100 pt-8 mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div>
                      ...
                      <input type="text" value={formData.sip} onChange={(e) => setFormData({...formData, sip: e.target.value})} className="w-full bg-[#F8FAFC] border border-gray-200 rounded-lg px-4 py-3 text-[14px] text-slate-800 focus:bg-white focus:border-[#253E6B] outline-none" />
                    </div>
                    <div>
                      <label className="block text-[13px] font-bold text-slate-700 mb-2">Spesialisasi</label>
                      <input type="text" value={formData.spesialisasi} onChange={(e) => setFormData({...formData, spesialisasi: e.target.value})} className="w-full bg-[#F8FAFC] border border-gray-200 rounded-lg px-4 py-3 text-[14px] text-slate-800 focus:bg-white focus:border-[#253E6B] outline-none" />
                    </div>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1 border-b border-gray-100 pb-2">
                      ...
                      <div className="flex items-center justify-between py-3">
                        ...
                        <ToggleSwitch checked={notifLaporan} onChange={() => setNotifLaporan(!notifLaporan)} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between py-3">
                        ...
                        <ToggleSwitch checked={notifAntrean} onChange={() => setNotifAntrean(!notifAntrean)} />
                      </div>
                      <div className="flex items-center justify-between py-3">
                        <p className="text-[14px] font-medium text-slate-700">Pesan & Info Sistem Medora</p>
                        <ToggleSwitch checked={notifSistem} onChange={() => setNotifSistem(!notifSistem)} />
                      </div>
                    </div>
                  </div>
