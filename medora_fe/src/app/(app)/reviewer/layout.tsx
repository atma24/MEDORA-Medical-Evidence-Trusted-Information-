...
export default function ReviewerLayout({ children }: { children: React.ReactNode }) {
  ...
  // Logika active state: Antrean Klaim tetap nyala kalau URL-nya di halaman Verifikasi
  const isActive = (path: string) => {
    if (path === '/reviewer/antrean-klaim' && pathname.includes('/reviewer/verifikasi')) {
      return true;
    }
      ...
      >
        <div>
          ...
          <div className="px-3 py-6">
            ...
            <nav className="space-y-1.5">
              ...
              <Link href="/reviewer/dashboard" className={getMenuClass('/reviewer/dashboard')}>
                <svg className={`w-5 h-5 shrink-0 ${isActive('/reviewer/dashboard') ? 'text-white' : 'text-[#243C62]'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>
                {isSidebarOpen && <span className="ml-3">Dashboard</span>}
              </Link>
              
              <Link href="/reviewer/antrean-klaim" className={getMenuClass('/reviewer/antrean-klaim')}>
                <svg className={`w-5 h-5 shrink-0 ${isActive('/reviewer/antrean-klaim') ? 'text-white' : 'text-[#243C62]'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                {isSidebarOpen && <span className="ml-3">Antrean Klaim</span>}
              </Link>
              
              <Link href="/reviewer/laporan" className={getMenuClass('/reviewer/laporan')}>
                <svg className={`w-5 h-5 shrink-0 ${isActive('/reviewer/laporan') ? 'text-white' : 'text-[#243C62]'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
        ...
        <div>
          <div className="px-3 mb-6">
            ...
            <nav className="space-y-1.5">
              {/* SUDAH DIUPDATE KE /pengaturan-akun */}
              <Link href="/reviewer/pengaturan-akun" className={getMenuClass('/reviewer/pengaturan-akun')}>
                <svg className={`w-5 h-5 shrink-0 ${isActive('/reviewer/pengaturan-akun') ? 'text-white' : 'text-[#243C62]'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
        ...
        <header className="h-24 bg-white border-b border-gray-200 flex items-center justify-between px-8 shrink-0">
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-3 text-right">
              <div>
                <p className="text-sm font-bold text-gray-800">Rauf</p>
                <p className="text-xs text-gray-500">Snowfreze@gmail.com</p>
              </div>
